---
id: "1ea9df60-86e1-5ad8-9bfe-629f3efa3dd4"
title: "EN-US Legal Translation"
title_cn: "英美法律翻译"
slug: "en-us-legal-translation"
url: "https://lawve.ai/@wouter-van-den-berg/skill/en-us-legal-translation"
author: "Wouter van den Berg"
author_slug: "wouter-van-den-berg"
category: "legal-drafting"
language: "en"
license: "MIT"
jurisdictions: "INT, US"
status: "active"
source_index: 75
...
原始名称：`EN-US Legal Translation`  
作者：Wouter van den Berg  
分类：legal-drafting  
来源：https://lawve.ai/@wouter-van-den-berg/skill/en-us-legal-translation  
来源类型：外部开源 Lawve skill

## README.md 中文翻译

任何需要将法律文件翻译成可供出版的英语的人的技能。给它一个任何语言的Word文件；返回保留格式的英文 .docx，提供比例如更高的质量Legora 的 DeepL 工具。

独特的特点：

* Skill 内置英语通用词典和每种语言的子词典（短语），涵盖并购、知识产权、IT/SaaS、金融、税务、诉讼、就业等。子词典完整覆盖西班牙语、德语、法语、汉语等11种语言；其他人也翻译得很好。
* 跟踪更改在接受/拒绝时正确读取，并且拼写错误被折叠。
* 定义按字母顺序重新排序。
* 页眉、页脚、脚注和注释已翻译。
* 默认情况下翻译为美国英语，根据要求翻译为英国英语。
* 贯穿始终的质量门。
* 需要几分钟而不是几秒钟，并且比简单的技能更大，但整体质量更高。

英国英语默认版本在这里：https://lawve.ai/en/skills/legal-translation-uk-english-wouter-van-den-berg
...
* Skill has built-in English general lexicons and per-language sub-lexicons (phrasebooks) covering M&A, IP, IT/SaaS, finance, tax, litigation, employment and more. Full sub-lexicon coverage for 11 languages including Spanish, German, French and Chinese; others translate really well too.
* Track changes read correctly when accepted/rejected, and typos are collapsed.
* Definitions reordered alphabetically.
* Headers, footers, footnotes and comments translated.
* Translates to US English by default, UK English on request.
* Quality gates throughout.
* Takes minutes rather than seconds and is larger than simpler skills, but overall quality is higher.

UK English default version here: https://lawve.ai/en/skills/legal-translation-uk-english-wouter-van-den-berg
...
re-serialization through Python's XML parsers can corrupt them in ways that look fine in XML but
...
| 6 | Post-process | `post_process.py` (terminology, spacing, US English, etc.) | `skill-docs/06-postprocess-and-reorder.md` |
...
7. **Chat-mode discipline.** This skill is designed assuming a working folder (e.g. Cowork mode) where `paragraphs.json`, `final/word/document.xml`, and the `.validate-state.json` checkpoint are real files persisted between steps. **In Chat mode (no working folder, no auto-managed todo list), the discipline must be self-enforced** — and the temptation to skim or compress is materially higher. **At session start, you MUST detect Chat-mode (see `skill-docs/01-setup-and-extract.md` Step 1a) and post the user-facing Chat-mode warning verbatim** — once per session, with `this document` vs `these N documents` phrasing matching the document count. **At Step 11a, pass `--mode chat` to `verify_diligence.py`** so the diligence report can append a Cowork recommendation if drift is detected. Specifically:
   ...
   - The 35-paragraph batch cap applies in Chat mode identically to Cowork. Do NOT bundle batches "to save context," "because the document is small," or "because the user is waiting." The validator's state-file check still fires; bypassing it is exactly the rationalization flagged in this section.
   ...
   The Chat-mode failure mode is the same as the Cowork-mode failure mode (drift, skim, paraphrase from memory) but with one extra failure surface: the operator can rationalize away the file-persistence requirements ("I have the JSON in context, I don't need to write it"). The validators are designed to catch defects the operator would not otherwise see; they only run on real files. Write the files.

If you find yourself rationalizing a deviation ("just this once," "the document is small," "I'll catch this in post-processing," "I'm in Chat so I can hold this in context"), STOP. The failure mode is exactly that rationalization.
...
## Target English variant: US English (do NOT ask the user)

This skill translates into **US English**. US is the hardcoded standard. **Do NOT ask the user "UK or US English?"** — US is the answer. The only situation in which UK English applies is when the user has *already* given an explicit instruction in their original prompt (phrases like "translate into UK English", "use British English"). If they haven't, proceed silently in US; you can mention "UK English available on request" in the delivery message rather than interrupting the translation to ask.
...
2. Search it for an unambiguous UK-English indicator: "UK English", "British English",
   "British spelling", "UK spelling", or a plainly equivalent phrase.
3. If and only if you find one, use UK English.
4. Otherwise — including when the user mentioned a UK party, a UK counterparty, a UK-based
   addressee, or any other context that feels British — use US English.

Do not infer "UK English" from context, from the client's nationality, from the document's
...
choose US. This re-check must happen at each of the following decision points; do not cache
a decision from earlier in the session:

- when drafting translations that contain variant-sensitive spelling or vocabulary;
- when invoking `post_process.py` (passes `--variant us` by default);
- when invoking `quality_check.py` (passes `--variant us` by default);
- when writing the delivery message to the user.

**How the user switches to UK English:** the user must include a direct instruction in their
request — phrases like "translate into UK English", "use British English", "British
spelling". When they do, apply UK English for that translation, and pass `--variant uk` to
both post-processing and quality-check scripts. If they didn't say it explicitly, US wins —
and you can mention in the delivery message that UK English is available on request rather
...
- **Jurisdiction-specific legal terms.** When the source document refers to institutions,
  ...
  those by their correct English rendering for that system — do not Americanize them to US
  equivalents just because the reader prefers US English, and do not Anglicize them to UK
...
preserve the established English rendering. Over-Anglicizing or over-Americanizing
...
ElementTree and lxml strip namespace declarations (xmlns:o, xmlns:v, etc.) when re-serializing.
...
rule says internal contract cross-references take "Section" in US English (the default)
or "Clause" in UK English, not "Article" (e.g. `本契約第3条 → "Section 3 of this Agreement"`
under the default US variant), but the translator had only consulted the
...
"Section" via regex to make the gate pass — solving the symptom without consulting the
...
(internal contract refs take "Section" in US, "Clause" in UK). The two layers are
...
**Cause**: `xml.etree.ElementTree` renames namespace prefixes when it re-serializes XML.
...
property-readers to honor the full falsy set, so this should be rare — but if a future
...
2. After `apply_translations_textmatch.py` and `post_process.py`: run
   ...
   normalizes to the same string, removes the del, and unwraps the ins. It also strips
...
2. The translator replaces the placeholders with the final English and leaves
   ...
   behavior of `apply_translations_textmatch.py` — a segment with
