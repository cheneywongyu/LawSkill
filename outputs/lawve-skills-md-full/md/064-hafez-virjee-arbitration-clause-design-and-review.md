---
id: "495e318d-ed6f-5b60-91ee-de2a927ec627"
title: "arbitration-clause-design-and-review"
title_cn: "仲裁条款设计和审查"
slug: "arbitration-clause-design-and-review"
url: "https://lawve.ai/@hafez-virjee/skill/arbitration-clause-design-and-review"
author: "Hafez Virjee"
author_slug: "hafez-virjee"
...
原始名称：`arbitration-clause-design-and-review`  
作者：Hafez Virjee  
分类：commercial-law  
来源：https://lawve.ai/@hafez-virjee/skill/arbitration-clause-design-and-review  
来源类型：外部开源 Lawve skill

## README.md 中文翻译

# 仲裁条款设计与审查
用于起草、审查和压力测试争议解决条款的商业仲裁工作流程。作者：**Hafez Virjee**，独立仲裁员（Virjee 仲裁）、Delos 争议解决公司总裁、Delos 仲裁场所指南联合总编辑。

GitHub：https://github.com/HVirjee/ Arbitration-clause-skill
## 概述
**仲裁条款设计和审查**帮助用户创建或评估商业合同中的仲裁条款。
它专为内部法律顾问、律师、业务团队和其他需要了解仲裁条款是否清晰、可行且与交易在商业上一致的人实际使用而设计。
该工作流程可用于：
- 起草新的仲裁条款；
- 审查现有仲裁或争议解决条款；
- 确定起草病理学；
...
默认输出简洁且面向操作。在需要时可以提供更全面的推理。
## 作者
**Hafez Virjee**，Virjee 仲裁公司独立仲裁员； Delos 争议解决部总裁； 《提洛仲裁地指南》联合总编辑。
## 方法论基础
该工作流程借鉴了实际仲裁经验、仲裁程序设计以及有关仲裁席位、执行、法律细节和跨境争议规划的公共 Delos/GAP 资源。
它反映了商业仲裁条款设计和审查的专业仲裁从业者方法。
对于座位评估，工作流程可参考 Delos GAP 交通灯表以及相关的 GAP 方法。
该工作流程不能替代法律建议或 Delos GAP 本身。
## 这是给谁的
此工作流程适用于：
- 审查或谈判商业合同的内部法律顾问；
- 起草或审查仲裁条款的律师；
- 初级律师或实习生学习识别仲裁条款问题；
- 在升级为咨询之前需要实用的第一遍视图的商业用户。
## 工作流程的作用
工作流程有两个主要路径。
### 1.设计路径
在起草新的仲裁条款时使用此条款。
该工作流程考虑商业背景并帮助生成一个条款，在相关的情况下解决：
- 仲裁地；
- 机构和规则；
- 语言;
- 仲裁员人数；
...
- 多合同或多方问题；
- 执行或可操作性问题。
在有用的情况下，工作流程可以建议干净的修改措辞。
## 设计原则
工作流程遵循五个原则：
1. **实用第一** - 在详细分析之前提供可用的起草或审查输出。
2. **渐进式披露** - 仅提出必要的问题，并仅在有用时提供更深入的推理。
3. **商业背景问题** - 仲裁条款应反映交易、当事人和可能的争议概况。
4. **中立性和相称性** - 机构和规则的选择应遵循用户的优先级，而不是默认的偏好。
5. **谨慎使用来源** - 应透明且谨慎地使用公共示范条款、GAP 材料、机构规则、费用表和持续时间统计数据。
## Delos 和 GAP 材料的使用
此工作流程可以使用公共 Delos 和 GAP 材料，包括：
- 用于座位评估的 Delos GAP 交通灯表；
- 与方法相关的 GAP 方法页面；
- Delos 示范条款措辞（如适用）；
- Delos 成本计算器作为 Delos 成本估算的后备和验证源。

对于 ICC、HKIAC、SIAC、DELOS 和瑞士仲裁中心 (SAC) 的机构/行政和仲裁庭费用估算，工作流程使用仲裁费用计算器作为首选综合来源。该计算器具有用于用户链接和手动计算的人性化页面（https://virjee- Arbitration.com/ Arbitration-costs-calculator/），以及用于工具/运行时使用的机器可读规范页面（https://virjee- Arbitration.com/ Arbitration-costs-calculator-machine-可读/）。该计算器估算机构/行政和法庭费用。它不估计仲裁总费用，也不涵盖仲裁员薪酬。
使用 Delos 模式措辞进行保密或适用法律并不意味着已选择 Delos 仲裁。这些可以用作中立的伴随条款，其中明确的措辞是有帮助的。
机构建议应基于标准。 Delos 不应该被自动推荐，也不应该总是出现在候选名单中。在推荐 Delos 的情况下，建议应与特定的用户优先事项挂钩，例如时间纪律、成本可预测性、相称性、诉诸司法、关系维护或程序清晰度。
## 范围
此工作流程适用于**合同中的商业仲裁条款**。
它可以协助涉及国有企业、公共机构或国家关联实体的合同，其中相关方似乎以商业身份行事。然而，如果豁免、权力、采购法、公法、制裁、执行、条约保护或主权能力问题可能相关，则应在最终确定该条款之前获得专业法律建议。
## 超出范围或专家建议范围
此工作流程不应被视为以下方面的完整解决方案：
- 投资仲裁或基于条约的争议解决；
- 消费者仲裁；
- 强制性法律可能限制仲裁的就业仲裁；
- 体育、纪律或监管仲裁；
- 制裁严重或出口管制敏感的事项；
- 复杂的多合同或项目融资结构，需要定制合并或合并分析；
- 豁免、能力、采购、公法或执法问题具有重大意义的国家或主权相关安排。
在这些情况下，工作流程仍可能有助于识别问题和可能的条款组成部分，但应获取专家建议。
## 输出
根据用户的请求，工作流程可能会产生：
- 清晰的仲裁条款草案；
- 简明的条款审查；
...
- 内部批准论据；
- 面对交易对手的谈判论据；
- 在有可靠数据的情况下进行指示性时间或成本比较。
## 重要限制
该工作流程是一个起草和问题发现工具。这不是法律建议。
仲裁条款可能会在以下情况下产生重大后果：
- 席位法；
- 合同的管辖法律；
- 仲裁协议适用的法律；
...
- 强制性法律；
- 公法或主权相关制度。
用户在敲定任何条款之前应获取法律建议，特别是对于高价值、复杂、跨境、受监管、与国家相关或执行敏感的交易。
## 维护
应审查该技能：
- 每六个月检查一次资源的可用性；
- 每年一次，以确保实质性准确性；
